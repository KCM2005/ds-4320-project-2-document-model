...
# Executive Summary
This project analyzed 30-day readmission risk for primary diagnoses in diabetic patients by gender, using descriptive analysis and logistic regression model. Descriptive analysis (which ranks diagnoses by frequency or most prevalent in the raw dataset) showed stroke and heart failure are the most frequent drivers, with females at 16.4% and 14.3% vs males at 16.1% and 13.9%, while males had higher rates for chronic bronchitis and pneumonia compared to females. In contrast, logistic regression model (which ranks diagnoses by Odds Ratios and identifies which diagnoses have the strongest statistical association with readmission, even if those diagnoses are rare in the raw dataset), revealed different high-risk but rarer primary conditions like secondary malignant neoplasm of respiratory/digestive systems and neoplasm of lymph nodes, though chronic bronchitis appeared as having the least predicted within 30-day readmission risk for both females and males. A heatmap was used to visualize diagnosis-gender pairs, with rows as diagnoses ordered by risk, columns as gender, and color intensity showing readmission rate, enabling rapid detection of disparities and gender gaps. The heatmap revealed that neoplasm of the respiratory and digestive system (49.9%), and neoplasm of the lymph nodes (~49.9%) have the highest mean predicted probability of readmission within 30-days for females and male, respectively, while chronic bronchitis has the least probability of readmission for females (~48.5%) and males (~48.7%). Overall, raw frequencies highlight high-volume drivers like stroke, whereas odds ratio-based modeling (logistic regression) flags high-risk rare conditions, confirming that readmission risk and gender differences vary substantially by diagnosis and supporting targeted, gender-focused interventions.
